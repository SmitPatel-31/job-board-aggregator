...
Published to GitHub Pages by the scrape workflows. Enable it once under
**Settings → Pages → Source: GitHub Actions**, then run **Update Job Listings**.

Self-hosting instead? See [deploy/README.md](deploy/README.md).
...
- **Automated pipeline**: GitHub Actions restores the live dataset → scrapes → merges → republishes site and data to Pages as one artifact
...
| Hosting  | GitHub Pages (app + dataset as one artifact)           |
...
# Chunked job data (jobs_chunk_*.json.gz + jobs_manifest.json) is NOT committed.
# It ships to GitHub Pages as a build artifact via .github/scripts/build-site.sh,
# so ~74MB of regenerated data never enters git history. Each run restores the
# previous dataset by fetching it back off the live site.
```
...
7. **Deploy**: `.github/scripts/build-site.sh` assembles `_site/` (app shell + chunks + trend log) and the workflow publishes it to GitHub Pages as an artifact. Nothing generated is committed, so history stays small.
...
| Tier | Schedule | Scope |
| ---- | -------- | ----- |
| Full scrape (`scrape-jobs.yml`) | daily, 07:33 UTC | all ~60k companies |
| Fast tier (`fresh-jobs.yml`) | every 6h at :17 | only companies that recently posted matching roles |

Both share a `job-data-pipeline` concurrency group so they never publish to Pages at the same time. Each run restores the previous dataset from the live site, scrapes, merges, and republishes.

Ashby rate-limits aggressively: a run from a single fixed IP logged 340+ HTTP 429s and spent ~90% of its wall time backing off. GitHub's runners rotate across a large shared IP pool, which is why the hosted runs are much faster than self-hosting on one address.
